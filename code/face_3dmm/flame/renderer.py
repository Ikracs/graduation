...
import face_3dmm.flame.utils as utils
...
class Renderer(nn.Module):
    def __init__(self, image_size, tris, uv_tris, uv_coords, uv_size=256):
        super(Renderer, self).__init__()
        self.image_size = image_size
        self.uv_size = uv_size

        uvcoords = uv_coords.unsqueeze(0)
        uvfaces = uv_tris.unsqueeze(0)
        faces = tris.unsqueeze(0)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        normals = utils.vertex_normals(vertices, self.faces.expand(batch_size, -1, -1))
        face_normals = utils.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = utils.vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        transformed_face_normals = utils.face_vertices(transformed_normals, self.faces.expand(batch_size, -1, -1))
    ...
    def render_normal(self, transformed_vertices, normals):
        ...
        # Attributes
        attributes = utils.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
    ...
    def world2uv(self, vertices):
        ...
        '''
        batch_size = vertices.shape[0]
        face_vertices = utils.face_vertices(vertices, self.faces.expand(batch_size, -1, -1)).clone().detach()
    ...
    def save_obj(self, filename, vertices, textures):
        ...
        '''
        utils.save_obj(filename, vertices, self.faces[0], textures=textures, uvcoords=self.raw_uvcoords[0],
