...
HEAD = BiSeNet.HEAD + BiSeNet.NECK
NECK = BiSeNet.NECK
...
class FramesDataset(Dataset):
    def __init__(self, datasets, sampler, expand_ratio=2.5, img_size=256, num_workers=4):
    ...
    def prepare_data(self, video):
        video_pth = os.path.join(PORTRAIT_DATASETS['data_root'], video)
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            src_msk = np.stack((seg2msk(src_seg, HEAD), seg2msk(src_seg, NECK)), axis=-1)
            dri_msk = np.stack((seg2msk(dri_seg, HEAD), seg2msk(dri_seg, NECK)), axis=-1)
        ...
        cid_msk = np.stack((seg2msk(cid_seg, HEAD), seg2msk(cid_seg, NECK)), axis=-1)

        data_item = {}

        data_item['src_img'] = cv2.resize(crop_image(src_img, face_bbox), self.img_size)
        data_item['src_msk'] = cv2.resize(crop_image(src_msk, face_bbox), self.img_size)

        data_item['dri_img'] = cv2.resize(crop_image(dri_img, face_bbox), self.img_size)
        data_item['dri_msk'] = cv2.resize(crop_image(dri_msk, face_bbox), self.img_size)

        data_item['cid_img'] = cv2.resize(crop_image(cid_img, cid_face_bbox), self.img_size)
        data_item['cid_msk'] = cv2.resize(crop_image(cid_msk, cid_face_bbox), self.img_size)
